package com.wy.arialena.modulecore.compress.utils;
...
import com.wy.arialena.modulecore.compress.exception.TakeException;
import com.wy.arialena.modulecore.utils.DeviceUtils;
...
public class TUriUtils {
    ...
    public static Uri checkCropUri(Context context, Uri uri, String suffix) throws TakeException {
        if (uri == null) {
            return getTempSchemeFileUri(context, suffix);
        }
        if (ContentResolver.SCHEME_FILE.equals(uri.getScheme())) {
            if (Build.VERSION.SDK_INT >= 29) {
                if (Environment.isExternalStorageEmulated()) {
    ...
    public static Uri getTempSchemeContentUri(@NonNull Context context, String suffix) {
        File file = new File(context.getExternalFilesDir(Environment.DIRECTORY_PICTURES), "/" + DeviceUtils.getUUID32() + (TextUtils.isEmpty(suffix) ? ".jpg" : suffix));
